...
int main() {
    ...
    int bingo_min_val = 100;
    int bingo_min_board = 0;
    ...
    for (int b = 0; b < 100; b++)
    {
        ...
        if (board_bingo < bingo_min_val) {
            bingo_min_val = board_bingo;
            bingo_min_board = b;
        }
    }

    int max_called = bingo_min_val;
    int max_called_val;
    int sum_uncalled = 0;
    ...
    for (int i = 0; i < 5; i++)
    {
        for (int j = 0; j < 5; j++)
        {
            if (taken[bingo_min_board][i][j] > max_called || taken[bingo_min_board][i][j] == 0)
            {
                sum_uncalled += N[bingo_min_board][i][j]; 
            }
            if (taken[bingo_min_board][i][j] == max_called) {
                max_called_val = N[bingo_min_board][i][j]; 
            }
        }
    }

    cout << max_called_val * sum_uncalled << endl;    
}
